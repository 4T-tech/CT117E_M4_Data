...
void LCD_Proc(void)               	/* LCD处理 */
{
  ...
  TIM3_GetCapture(usCapt);
  sprintf((char *)ucLcd, " WID:%04u  B2: %02u%% ", usCapt[0]-usCapt[1], ucDuty);
  LCD_DisplayStringLine(Line6, ucLcd);
  sprintf((char *)ucLcd, " PER:%04u  FRE:%04u ", usCapt[0], 1000000/usCapt[0]);
  LCD_DisplayStringLine(Line7, ucLcd);
  TIM2_GetCapture(usCapt);
  sprintf((char *)ucLcd, " R40:%04u  FRE:%04u ", usCapt[0], 1000000/usCapt[0]);
