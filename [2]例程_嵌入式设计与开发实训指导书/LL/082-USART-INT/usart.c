...
void MX_USART1_UART_Init(void)
{
  ...
  */
  ...
  LL_USART_EnableIT_RXNE(USART1);    /* 允许USART1接收中断 */
  LL_USART_EnableIT_IDLE(USART1);    /* 允许USART1空闲中断 */
...
void UART_Transmit(uint8_t *ucData, uint8_t ucSize)
{
  for (uint8_t i=0; i<ucSize; i++) {	/* 等待发送寄存器空 */
    ...
    LL_USART_TransmitData8(USART1, *ucData++);
  }							                  		/* 发送8位数据 */
}
...
{                                     /* 接收寄存器不空 */
  if (LL_USART_IsActiveFlag_RXNE(USART1) == 1) {
    ucData[ucUno] = LL_USART_ReceiveData8(USART1);
    if (++ucUno == ucSize) {
      ucUno = 0;
      return 0;                       /* 接收数据完成返回0 */
    }
