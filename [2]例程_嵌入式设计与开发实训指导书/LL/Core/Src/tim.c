...
void MX_TIM1_Init(void)
{
  ...
  LL_TIM_CC_EnableChannel(TIM1, LL_TIM_CHANNEL_CH1N);/* 允许TIM1_CH1N */
...
void MX_TIM2_Init(void)
{
  ...
  */
  ...
  LL_TIM_CC_EnableChannel(TIM2, LL_TIM_CHANNEL_CH1 | LL_TIM_CHANNEL_CH2);
  								              			/* 允许TIM2_CH1和CH2 */
  LL_TIM_EnableCounter(TIM2); 				/* 允许TIM2 */
...
void MX_TIM3_Init(void)
{
  ...
  */
  ...
  LL_TIM_CC_EnableChannel(TIM3, LL_TIM_CHANNEL_CH1 | LL_TIM_CHANNEL_CH2);
  											/* 允许TIM3_CH1和CH2 */
	LL_TIM_EnableCounter(TIM3); 				/* 允许TIM3 */
...
void MX_TIM4_Init(void)
{
  ...
  LL_TIM_CC_EnableChannel(TIM4, LL_TIM_CHANNEL_CH1);/* 允许TIM4_CH1 */
  LL_TIM_EnableCounter(TIM4); 				/* 允许TIM4 */
...
{
  LL_TIM_SetAutoReload(TIM1, usAuto);	  	/* 设置TIM1自动重装值 */
}
void TIM4_SetAutoReload(uint16_t usAuto) 	/* 设置TIM4自动重装值 */
{
  LL_TIM_SetAutoReload(TIM4, usAuto);	  	/* 设置TIM4自动重装值 */
}
void TIM1_SetCompare1(uint16_t usComp) 		/* 设置TIM1输出比较值1 */
{
  LL_TIM_OC_SetCompareCH1(TIM1, usComp);	/* 设置TIM1_CH1比较值 */
}
void TIM4_SetCompare1(uint16_t usComp) 		/* 设置TIM4输出比较值1 */
{
  LL_TIM_OC_SetCompareCH1(TIM4, usComp);	/* 设置TIM4_CH1比较值 */
}
void TIM2_GetCapture(uint16_t *pusBuf)  	/* 获取TIM2输入捕获值 */
{
  pusBuf[0]=LL_TIM_IC_GetCaptureCH1(TIM2)+1;
  pusBuf[1]=LL_TIM_IC_GetCaptureCH2(TIM2)+1;
}
void TIM3_GetCapture(uint16_t *pusBuf)  	/* 获取TIM3输入捕获值 */
{
  pusBuf[0]=LL_TIM_IC_GetCaptureCH1(TIM3)+1;
  pusBuf[1]=LL_TIM_IC_GetCaptureCH2(TIM3)+1;
}
/* USER CODE END 1 */
